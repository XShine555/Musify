...
public sealed class UserEndpointsTests(ApiTestFixture fixture)
{
    ...
    public async Task PostUsers_NewUser_ReturnsCreatedWithLocationHeader()
    {
        ...
        var response = await client.PostAsJsonAsync("/users", new CreateUserRequest(userId, "Jane Doe", "Jane", "Doe"));

        Assert.Equal(HttpStatusCode.Created, response.StatusCode);
        Assert.Equal($"/users/{userId}", response.Headers.Location?.OriginalString);
        var body = await response.Content.ReadFromJsonAsync<UserApplicationResponse>();
        Assert.NotNull(body);
    ...
    public async Task PostUsers_BlankName_ReturnsValidationProblem()
    {
        ...
        var response = await client.PostAsJsonAsync("/users", new CreateUserRequest(Random.Shared.NextInt64(1, long.MaxValue), "", null, null));
    ...
    public async Task PostUsers_DuplicateId_ReturnsConflict()
    {
        var client = fixture.CreateAnonymousClient();
        var userId = Random.Shared.NextInt64(1, long.MaxValue);
        await client.PostAsJsonAsync("/users", new CreateUserRequest(userId, "First", null, null));

        var response = await client.PostAsJsonAsync("/users", new CreateUserRequest(userId, "Second", null, null));
    ...
    public async Task GetUserById_ExistingUser_ReturnsIt()
    {
        var client = fixture.CreateAnonymousClient();
        var userId = Random.Shared.NextInt64(1, long.MaxValue);
        await client.PostAsJsonAsync("/users", new CreateUserRequest(userId, "Findable User", null, null));
    ...
    public async Task GetUserById_MissingUser_ReturnsNotFound()
    {
        ...
        var response = await client.GetAsync($"/users/{Random.Shared.NextInt64(1, long.MaxValue)}");
