...
public sealed class GetAlbumsQueryHandlerTests : HandlerTestBase
{
    ...
    public async Task Handle_NoTitleFilter_ReturnsAllAlbumsPaged()
    {
        ...
        var result = await CreateHandler().Handle(new GetAlbumsQuery(Title: null, PageNumber: 1, PageSize: 10), CancellationToken.None);
    ...
    public async Task Handle_TitleFilter_ReturnsOnlyMatchingAlbums()
    {
        ...
        var result = await CreateHandler().Handle(new GetAlbumsQuery(Title: "rock", PageNumber: 1, PageSize: 10), CancellationToken.None);
    ...
    public async Task Handle_PageSizeSmallerThanTotal_ReportsHasNextPage()
    {
        ...
        var result = await CreateHandler().Handle(new GetAlbumsQuery(Title: null, PageNumber: 1, PageSize: 2), CancellationToken.None);
