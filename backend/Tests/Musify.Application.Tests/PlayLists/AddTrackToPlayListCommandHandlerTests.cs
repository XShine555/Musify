...
public sealed class AddTrackToPlayListCommandHandlerTests : HandlerTestBase
{
    ...
    public async Task Handle_FirstTrack_AddsItAtPositionZero()
    {
        ...
        var result = await CreateHandler().Handle(new AddTrackToPlayListCommand(owner.Id, playList.Id, track.Id), CancellationToken.None);

        Assert.False(result.IsError);
        var link = Assert.Single(await Database.PlayListHasTracks.ToListAsync());
    ...
    public async Task Handle_PlayListAlreadyHasTracks_AppendsAtNextPosition()
    {
        ...
        var result = await CreateHandler().Handle(new AddTrackToPlayListCommand(owner.Id, playList.Id, newTrack.Id), CancellationToken.None);

        Assert.False(result.IsError);
        var newLink = await Database.PlayListHasTracks.SingleAsync(link => link.TrackId == newTrack.Id);
    ...
    {
        var result = await CreateHandler().Handle(new AddTrackToPlayListCommand(1, Guid.NewGuid(), Guid.NewGuid()), CancellationToken.None);
    ...
    public async Task Handle_NotOwner_ReturnsUnauthorized()
    {
        ...
        var result = await CreateHandler().Handle(new AddTrackToPlayListCommand(stranger.Id, playList.Id, Guid.NewGuid()), CancellationToken.None);
    ...
    public async Task Handle_TrackMissing_ReturnsNotFound()
    {
        ...
        var result = await CreateHandler().Handle(new AddTrackToPlayListCommand(owner.Id, playList.Id, Guid.NewGuid()), CancellationToken.None);
