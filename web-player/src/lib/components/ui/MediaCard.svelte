...
<script lang="ts">
	...
	let { href, title, subtitle, index = 0, oncontextmenu, art }: Props = $props();
</script>
...
>
	...
	<div class="mt-2.5 truncate text-fg transition-colors group-hover:text-accent-soft">{title}</div>
	{#if subtitle}
		<div class="mt-0.5 truncate text-sm text-fg-3">{subtitle}</div>
	{/if}
</a>
