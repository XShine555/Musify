...
>
	<div class="relative aspect-square overflow-hidden rounded-art shadow-art">
		...
		<button
			...
			class="absolute right-2.5 bottom-2.5 grid h-9.5 w-9.5 place-items-center rounded-full border border-white/12 bg-ink/76 text-fg opacity-0 backdrop-blur-[8px] transition-opacity duration-150 group-hover/tile:opacity-100"
		>
			<Play class="h-3.5 w-3.5" fill="currentColor" />
		</button>
	</div>
	<div class="mt-3.25 truncate text-[13px] font-semibold tracking-[-0.01em] text-fg">
		{mix.title}
	</div>
	{#if mix.subtitle}
		<div class="mt-1 truncate text-[11.5px] text-fg-3">{mix.subtitle}</div>
	{/if}
</a>
