...
Cada carpeta de nivel superior es una solución/repo independiente.

## Musify.WebApi/WebApi  (`:5111`)
...
- **CORS** totalmente abierto **solo en Development** (para el `player.html` y front local).
- Traduce `Ardalis.Result` → HTTP en `ResultHttpExtensions`.

**Propósito/porqué**: separar la cara pública (autorización + metadatos) del procesado y del servido de bytes.

## MusifyBackend  (Domain + Application + Infrastructure)
...
**Porqué**: la lógica de negocio no conoce EF, S3 ni RabbitMQ; solo interfaces. Reutilizable por WebApi y Worker.
...
## Musify.StreamingGateway/StreamingGateway  (`:8081`)
...
## Raíz del workspace (no versionada)

- **`deploy/seaweedfs/`** — `docker-compose.yml` + `s3.conf` para levantar SeaweedFS.
- **`player.html`** — consola de pruebas (un solo archivo): login con token, CRUD de playlists, añadir/quitar canciones, listar y reproducir. Ver [development.md](development.md). (Pendiente: hoy usa dash.js; con el cambio a `.m4a` debe reproducir el fichero directo por `<audio>`/range.)
