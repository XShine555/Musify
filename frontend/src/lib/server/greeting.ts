const GREETINGS_BY_HOUR: { until: number; pool: string[] }[] = [
	...
	{ until: 12, pool: ['Buenos días', 'Que empiece bien la mañana', 'Un nuevo día, una nueva lista'] },
...
export function pickGreeting(hour: number): string {
	const pool = GREETINGS_BY_HOUR.find((slot) => hour < slot.until)?.pool ?? GREETINGS_BY_HOUR[0].pool;
